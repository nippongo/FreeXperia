...
#define CMD_SPACE_REG 0x40
#define LEAVE_ROM_DELAY_MS 250
...
static bool bq27520_check_if_sealed(struct bq27520_data *bd)
{
	...
	return !!(bd->control_status & SEALED_MASK);
}
				   ...
				   struct bq27520_golden_info *gi)
{
	...
	if (!rc) {
		/* TI recommend a 2 ms delay before a data flash block read */
		msleep(2);
	...
	if (rc) {
		 ...
		 */
		gi->fw_compatible_version = ror16(gi->fw_compatible_version, 8);
		gi->golden_file_version = ror16(gi->golden_file_version, 8);
	} else {
...
static int bq27520_last_step_production(struct bq27520_data *bd)
{
	 ...
	 */
	return bq27520_write_control(bd, SUB_CMD_IT_ENABLE);
...
{
	unsigned int poll_cnt = POLL_QEN_TIMEOUT_MS / POLL_QEN_PERIOD_MS;
	int rc = 0;
	unsigned int i;
...
static int bq27520_write_temperature(struct bq27520_data *bd, int temp)
{
	...
	/* The QEN bit must be set before writing temperature after
	 * a firmware upload. Otherwise it will have no effect.
	 */
	if (bd->run_init_after_rom) {
		rc = bq27520_wait_for_qen_set(bd);
		if (rc) {
			pr_err("%s: QEN not set when writing temperature."
			       " rc=%d\n", BQ27520_NAME, rc);
			return rc;
		}
	}
					...
					int type, int temp)
{
	...
	bq27520_write_temperature(bd, temp);

	bq27520_read_bat_flags(&bd->bat_ps);
	/* Only set BAT_INSERT if battery is not detected.
