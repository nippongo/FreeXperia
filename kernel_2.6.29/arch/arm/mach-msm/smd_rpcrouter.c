...
static void analyze_pm(void)
{
	struct tmp_smd_channel {
	...
	struct tmp_smd_channel*ch = (void*)smd_channel;
	int i;
	int send_i = 0;
	uint32_t* rpc_sendbuf = (void*)ch->send_buf;
	struct rr_header* hdr;
	uint32_t* data;
	uint32_t last_pm;
	uint32_t last_xid;
	printk("rpc_sendbuf=%p\n", rpc_sendbuf);
	for(i=0; i<0x1000; i++){
	...
	while(hdr->version == RPCROUTER_VERSION){
		if(hdr->type == 1){ //CMD_DATA
			data = (void*)(hdr+1);
			if(last_pm < (data[0] & 0xff0000))
				last_pm = data[0] & 0xff0000;
			if(last_xid < (data[1] >> 24))
				last_xid = data[1] >> 24;
		}
		hdr = (void*)(((unsigned*)hdr) + ((hdr->size/4) + 8));
	}
	printk("last_pm=%x last_xid=%x\n", last_pm, last_xid);
	atomic_set(&pm_mid, (last_pm >> 16) & 0xff);
	atomic_set(&next_xid, (last_xid >> 24) & 0xff);
}
...
static void add_default_services(void)
{
	...
	static const struct service services[] = {
		{0x30000081, 0x00010002}, //PING_MDM_RPC
		{0x30000080, 0x00010001}, //PMAPP_OTG_REMOTE
		{0x30000061, 0x00010001}, //PM_LIB
		{0x30000003, 0xd03123cc}, //WMS
		{0x30000060, 0x00010002}, //PMAPP_GEN
		{0x3000005f, 0x00010001}, // DSUCSDAPPIF_APIS
		{0x30000000, 0xfd767954}, // zdzihu: 2.1 baseband .52 fix
		{0x3000001d, 0x00010001}, // PDSM_ATL
		{0x3000005b, 0xa60286b0}, // PDAPI
		{0x3000003c, 0xbeb78360}, // zdzihu: 2.1 baseband .52 fix
		{0x3000001b, 0x90258183}, // UIM
		{0x3000001a, 0x00010003}, // CHARGER
		{0x30000019, 0x821a1945}, // zdzihu: 2.1 fix
		{0x30000016, 0xd6bb93e6}, // GSDI_EXP
		{0x30000014, 0xcf2ba98e}, // MVS
		{0x30000012, 0x00010001}, // PBMLIB
		...
		{0x30000048, 0x00010004}, // TIME_REMOTE_ATOM
		{0x30000064, 0x00010002}, // HSU_MDM_APIS
		{0x30eeeeee, 0x00010001}, // zdzihu: 2.1 baseband .52 fix
	};
	union rr_control_msg *msg = (void*)r2r_buf;

	if(!first) return;

	first = 0;
	initialized = 1;
	msg->cmd = RPCROUTER_CTRL_CMD_NEW_SERVER;
	msg->srv.pid = RPCROUTER_PID_REMOTE;
	msg->srv.cid = 1;
	for(i=0; i<sizeof(services)/sizeof(struct service); i++){
		msg->srv.prog = services[i].prog;
		msg->srv.vers = services[i].vers;
		process_control_msg((void*) r2r_buf, 0x14);
	}
...
static int msm_rpcrouter_probe(struct platform_device *pdev)
{
	...
	queue_work(rpcrouter_workqueue, &work_read_data);
#if 1
	add_default_services(); //already initialized by original kernel..;
#endif
	return 0;
